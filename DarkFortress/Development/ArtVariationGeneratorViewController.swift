...
final class ArtVariationGeneratorViewController: UIViewController {
    var onArtworkSaved: (([String: Data]) throws -> Void)?
    private let picker = UIButton(type: .system)
    private let intensity = UISegmentedControl(items: ["25%", "50%", "75%", "100%"])
    ...
    private func configureLayout() {
        ...
        intensity.selectedSegmentIndex = 0
        ...
        intensityLabel.text = "Variation Intensity"
        ...
        let content = UIStackView(arrangedSubviews: [picker, currentTitle, currentGrid, divider,
            intensityLabel, intensity, buttons, status, candidateTitle, candidateGrid])
    ...
    private func updateControls() {
        ...
        picker.menu = UIMenu(children: ArtCatalog.definitions.sorted { $0.name < $1.name }.map { definition in
    ...
    @objc private func generateVariation() {
        ...
        let amount = Double(intensity.selectedSegmentIndex + 1) / 4
